 ...
 * - INTEREST_CRON_DEBUG=1 : Run every minute for testing (overrides all other settings)
...
const startInterestSchedulers = () => {
  ...
  // In DEBUG mode, force both jobs to run every minute regardless of explicit env
  ...
  if (debug) {
    console.warn('⚠️  DEBUG MODE: Interest processors set to run EVERY MINUTE');
